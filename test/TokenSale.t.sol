...
contract TokenSaleTest is Test {
    ...
    function setUp() public {
        ...
        vm.startPrank(owner);
        mockERC20Token = new MockERC20Token("TestToken", "TTK");
    ...
    function testContractInitialization() public {
        ...
        assertEq(mockERC20Token.name(), "TestToken", "Incorrect token name");
        assertEq(mockERC20Token.symbol(), "TTK", "Incorrect token symbol");
        assertEq(
            address(tokenSale.tokenContract()),
            address(mockERC20Token),
            "TokenSale contract does not have correct token address"
        );
    ...
    function testContributionWithinLimitsPublicSale() public {
        ...
        assertEq(
            uint256(tokenSale.getCurrentSaleState()),
            uint256(TokenSale.SaleState.PreSale),
        ...
        assertEq(
            uint256(tokenSale.getCurrentSaleState()),
            uint256(TokenSale.SaleState.PublicSale),
            "State should be PublicSale after time warp"
        );
        ...
        uint256 expectedTokenAmount = contributionAmount * tokenRatePerEth;
        assertEq(
            mockERC20Token.balanceOf(user2), expectedTokenAmount, "Token balance should match the expected token amount"
        );
    ...
    function testTransitionFromPreSaleToPublicSale() public {
        ...
        assertEq(
            uint256(tokenSale.getCurrentSaleState()),
            uint256(TokenSale.SaleState.PreSale),
            "State should be PreSale initially"
        );
    ...
    function testTokenDistributionByOwner() public {
        ...
        assertEq(
            finalBalance, initialBalance + distributionAmount, "Token distribution should increase recipient's balance"
        );
    ...
    function testRefundFunctionalityPreSaleCapNotMet() public {
        ...
        assertEq(
            user1.balance, initialEthBalance + contributionAmount, "Refund should be equal to the contribution amount"
        );
    }
}
